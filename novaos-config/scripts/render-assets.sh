...
#
# PRE-REQUISITE: Run scripts/build-grub-fonts.sh FIRST to generate
# the GRUB .pf2 font files needed by grub-theme/novaos/theme.txt.
# ----------------------------------------------------------------------
...
echo "[render] GRUB background (1920x1080)"
render "$WALLPAPER_DIR/novaos-wallpaper.svg" 1920 1080 \
       "$ROOT/grub-theme/novaos/background.png"
# Also copy to the assets dir so install-branding.sh can find it
mkdir -p "$ROOT/grub-theme/novaos"
echo "  ✓ GRUB background.png placed in grub-theme/novaos/"
